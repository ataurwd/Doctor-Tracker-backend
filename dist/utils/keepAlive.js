...
const initKeepAlive = () => {
    const targetBaseUrl = process.env.RENDER_EXTERNAL_URL ||
        process.env.SERVER_URL ||
        process.env.PING_URL;
    if (!targetBaseUrl) {
        console.log('[Keep-Alive] No RENDER_EXTERNAL_URL or SERVER_URL detected. Keep-alive self-ping is idle. (Render sets RENDER_EXTERNAL_URL automatically in production).');
        return;
    }
    ...
    const intervalMs = parseInt(process.env.KEEP_ALIVE_INTERVAL_MS || '720000', 10);
    const intervalMinutes = Math.round(intervalMs / 60000);
    console.log(`[Keep-Alive] Active! Pinging ${healthUrl} every ${intervalMinutes} minutes to prevent Render free-tier sleep.`);
    const pingServer = async () => {
        try {
            ...
            if (response.ok) {
                console.log(`[Keep-Alive] Ping successful at ${new Date().toLocaleTimeString()} (Status: ${response.status})`);
            }
            else {
                console.warn(`[Keep-Alive] Ping returned unexpected status: ${response.status} at ${new Date().toLocaleTimeString()}`);
            }
        }
        catch (error) {
            console.warn(`[Keep-Alive] Ping attempt error: ${error.message} (Will retry in ${intervalMinutes}m)`);
        }
    };
